...
export const useWorkoutStore = create<WorkoutState>((set, get) => ({
  ...
  beginSession: () => {
    ...
    set({
      ...
      exerciseIndex: 0,
      isGetReady: true,
      timer: makeTimerSnapshot(GET_READY_DEFAULT),
  ...
  finishGetReady: () => {
    ...
    const duration = list[exerciseIndex]?.duration ?? GET_READY_DEFAULT;
  ...
  resumeWorkout: () => {
    ...
    set({
      timer: resumeSnapshot(timer),
      isPaused: false,
      pausedAt: null,
      pausedDurationMs: pausedDurationMs + added,
    });
  },
    ...
    if (section === "warmup") {
      if (nextIdx < list.length) {
        set({
          ...
          isGetReady: true,
          timer: makeTimerSnapshot(GET_READY_SHORT),
        });
      } else {
        set({
          section: "exercise",
      ...
      if (nextIdx < list.length) {
        ...
        if (nextEx.isRest) {
          ...
          set({
            exerciseIndex: nextIdx,
            isGetReady: false,
            timer: makeTimerSnapshot(nextEx.duration),
          });
        } else {
          const comingFromRest = list[exerciseIndex]?.isRest ?? false;
          set({
            exerciseIndex: nextIdx,
            isGetReady: true,
            timer: makeTimerSnapshot(comingFromRest ? GET_READY_SHORT : GET_READY_DEFAULT),
          });
        }
      } else {
    ...
    if (isGetReady) {
      ...
      if (exerciseIndex > 0) {
        ...
        set({
          ...
          isGetReady: false,
          timer: makeTimerSnapshot(list[prevIdx].duration),
        });
        ...
        set({
          ...
          exerciseIndex: lastIdx,
          isGetReady: false,
          timer: makeTimerSnapshot(exList[lastIdx].duration),
        });
      }
      return;
    }
  ...
  getOverallExerciseNumber: () => {
    ...
    if (section === "exercise") {
      return (
        workout.warmups.length +
        workout.exercises.slice(0, exerciseIndex + 1).filter((e) => !e.isRest)
          .length
      );
    }
    if (section === "cooldown") {
