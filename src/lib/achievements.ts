...
export const ACHIEVEMENT_CHECKS: Record<string, CheckFn> = {
  "flag-planter": (s) =>
    new Set(s.completed.map((c) => categoryIndex(c.workoutId))).size >= 4,
  ...
  "planet-core": (s) =>
    s.completed.filter((c) => categoryIndex(c.workoutId) === 3).length >= 20,
  ...
  "refuel-day": (s) => s.refuelClaimed >= 1,
  // Level 2
  ...
  "high-roller": (s) =>
    s.levelCycles[1] >= 1 && s.levelCycles[2] >= 1 && s.levelCycles[3] >= 1,
