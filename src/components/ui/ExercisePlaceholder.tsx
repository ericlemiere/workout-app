...
export function ExerciseImage({ src, alt, className = '', category = 'exercise' }: Props) {
  ...
  if (failed || !src) {
    return (
      <div
        className={`bg-gradient-to-br ${gradient} flex flex-col items-center justify-center ${className}`}
      >
        <span className="text-5xl mb-2 opacity-60">{icon}</span>
        <span className="text-slate-400 text-sm text-center px-4 leading-snug">{alt}</span>
      </div>
    )
  }
